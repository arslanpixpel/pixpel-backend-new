...
cron.schedule("0 * * * *", async () => {
  try {
    ...
    if (nfts.rows.length) {
      ...
      console.log(polygonInsuranceNfts, "polygonInsuranceNfts");
      ...
      for (const nft of polygonInsuranceNfts) {
        ...
        console.log(updatedSecondaryOwners, "updatedSecondaryOwners");
        if (updatedSecondaryOwners.length !== nft.secondary_owner.length) {
          ...
          await query(
            "UPDATE nftmarket SET seller = $1, listing = $2 WHERE id = $3",
            [nft.primary_owner, true, nft.id]
          );
        }
      }
    }
